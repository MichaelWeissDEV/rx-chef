...
fn test_bzip2_decompress_basic() {
    ...
    let compressed = compress_op.run(
        test_data.to_vec(),
        &[ArgValue::Num(9.0), ArgValue::Num(30.0)],
    );

    assert!(
        compressed.is_ok(),
        "Should be able to compress test data first"
    );
    ...
    if let Ok(decompressed) = result {
        assert!(
            ...
            "Decompressed output should not be empty"
        );
...
fn test_bzip2_decompress_low_memory() {
    ...
    assert!(
        result.is_ok(),
        "Bzip2 decompression with low-memory should succeed"
    );
    if let Ok(decompressed) = result {
        assert_eq!(
            decompressed, test_data,
...
fn test_bzip2_decompress_roundtrip() {
    ...
    for test_data in test_cases {
        ...
        let compressed = compress_op.run(
            test_data.clone(),
            &[ArgValue::Num(5.0), ArgValue::Num(30.0)],
        );
