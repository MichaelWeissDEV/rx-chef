...
fn test_bzip2_compress_different_block_sizes() {
    ...
    for block_size in [1.0, 5.0, 9.0] {
        ...
        assert!(
            result.is_ok(),
            "Bzip2 compression with block size {} should succeed",
            block_size
        );
        if let Ok(compressed) = result {
            assert!(
...
fn test_bzip2_compress_already_compressed() {
    ...
    assert!(
        result.is_ok(),
        "Bzip2 compression should handle already compressed data"
    );
}
...
fn test_bzip2_compress_invalid_block_size() {
    ...
    assert!(
        result.is_ok(),
        "Bzip2 compression should handle invalid block size"
    );
}
