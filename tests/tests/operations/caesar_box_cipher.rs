...
fn test_caesar_box_cipher_basic() {
    ...
    if let Ok(output) = result {
        ...
        assert_eq!(
            output_str, "HLOOLELWRD",
            "Caesar box cipher output should match expected"
        );
    }
}
...
fn test_caesar_box_cipher_height_1() {
    ...
    assert!(
        result.is_ok(),
        "Caesar box cipher with height 1 should succeed"
    );
    if let Ok(output) = result {
        let output_str = String::from_utf8_lossy(&output);
...
fn test_caesar_box_cipher_height_3() {
    ...
    assert!(
        result.is_ok(),
        "Caesar box cipher with height 3 should succeed"
    );
    if let Ok(output) = result {
        let output_str = String::from_utf8_lossy(&output);
...
fn test_caesar_box_cipher_large_height() {
    ...
    if let Ok(output) = result {
        ...
        assert_eq!(
            output_str, "HELLO",
            "Large height should work like height 1"
        );
    }
}
