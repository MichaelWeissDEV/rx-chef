...
|---:|---|---|:---:|---|---|:---:|---|
| 1 | Delimiter | `Enum` | no | `Line feed` | Line feed, CRLF, Space, Comma, Semi-colon | no | Character(s) to split the input on (Line feed, CRLF, Space, Comma, Semi-colon) |
...
The implementation is in `src/operations/filter.rs` and declares `String` input and `String` output. The operation module owns conversion and domain-error rules; registry resolution, argument validation, input-requirement enforcement, tracing, and output validation are performed by `rxchef::execution`.
...
```console
rxchef run "Filter" --input-file input.bin --output-file output.bin
```
...
Correctness tests:
- tests/tests/operations/filter.rs
