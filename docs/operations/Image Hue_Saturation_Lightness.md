...
The implementation is in `src/operations/image_hue_saturation_lightness.rs` and declares `Bytes` input and `Bytes` output. Its operation module owns the conversion and error rules; every public frontend invokes it through `rxchef::execution`.
