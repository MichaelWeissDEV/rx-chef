...
|---:|---|---|:---:|---|---|:---:|---|
| 1 | Password | `String` | no | `<empty>` | — | yes | The password used to derive the LS47 key |
...
Source module: `src/operations/ls47_decrypt.rs`. Execution uses `rxchef::execute`; CLI, recipes, and the stdio server do not carry separate operation logic.

## Examples

```console
printf 'input' | rxchef run "LS47 Decrypt"
```

For file or binary input use `rxchef run "LS47 Decrypt" --input-file INPUT --output-file OUTPUT`.

## Pipeline usage

```console
printf 'input' | rxchef pipe "LS47 Decrypt" to_base64
```
