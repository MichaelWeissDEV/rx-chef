...
| 2 | Length | `Integer` | no | `5` | — | no | Number of bytes to take (negative reverses direction) |
...
Source module: `src/operations/take_bytes.rs`. Execution uses `rxchef::execute`; CLI, recipes, and the stdio server do not carry separate operation logic.
