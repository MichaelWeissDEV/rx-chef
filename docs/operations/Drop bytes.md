...
```console
printf 'input' | rxchef run "Drop bytes"
```

For file or binary input use `rxchef run "Drop bytes" --input-file INPUT --output-file OUTPUT`.

## Pipeline usage

```console
printf 'input' | rxchef pipe "Drop bytes" to_base64
```
