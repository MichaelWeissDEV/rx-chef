...
The implementation is in `src/operations/parse_unix_file_permissions.rs` and declares `String` input and `String` output. The operation module owns conversion and domain-error rules; registry resolution, argument validation, input-requirement enforcement, tracing, and output validation are performed by `rxchef::execution`.
...
```console
rxchef run "Parse UNIX file permissions" --input-file input.bin --output-file output.bin
```
