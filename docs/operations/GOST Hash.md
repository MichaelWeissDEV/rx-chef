...
| 2 | Digest length | `Integer` | no | `256` | — | no | The length of the digest to produce (only for Streebog). |
...
Source module: `src/operations/gost_hash.rs`. Execution uses `rxchef::execute`; CLI, recipes, and the stdio server do not carry separate operation logic.
