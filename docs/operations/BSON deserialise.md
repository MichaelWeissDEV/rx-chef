...
The implementation is in `src/operations/bson_deserialise.rs` and declares `Bytes` input and `JSON` output. Its operation module owns the conversion and error rules; every public frontend invokes it through `rxchef::execution`.

## Examples

```console
printf 'input' | rxchef run "BSON deserialise"
```

For file or binary input use `rxchef run "BSON deserialise" --input-file INPUT --output-file OUTPUT`.

## Pipeline usage

```console
printf 'input' | rxchef pipe "BSON deserialise" to_base64
```
...
Correctness:
- tests/tests/operations/bson_deserialise.rs
