...
The implementation is in `src/operations/md2.rs` and declares `Bytes` input and `String` output. The operation module owns conversion and domain-error rules; registry resolution, argument validation, input-requirement enforcement, tracing, and output validation are performed by `rxchef::execution`.
...
```console
rxchef run "MD2" --input-file input.bin --output-file output.bin
```
