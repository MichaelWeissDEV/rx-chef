 ...
 * License:     Apache-2.0
 * Description: Implementation of the Vigenre Encode operation.
 * -----------------------------------------------------------------------------
 */

use crate::operation::{ArgSchema, ArgValue, DataType, Operation, OperationError};

/// Vigenre Encode operation
pub struct VigenereEncodeOp;

impl Operation for VigenereEncodeOp {
    ...
    fn run(&self, input: Vec<u8>, args: &[ArgValue]) -> Result<Vec<u8>, OperationError> {
        ...
        for (i, c) in input_str.chars().enumerate() {
            if c.is_ascii_lowercase() {
                ...
                let encoded_index = (key_index + msg_index) % 26;
                output.push((b'a' + encoded_index) as char);
            } else if c.is_ascii_uppercase() {
                let key_char = key_bytes[(i - fail) % key_bytes.len()];
                let key_index = key_char - b'a';
                let msg_index = (c.to_ascii_lowercase() as u8) - b'a';
                let encoded_index = (key_index + msg_index) % 26;
                output.push(((b'a' + encoded_index) as char).to_ascii_uppercase());
            } else {
                output.push(c);
                fail += 1;
            }
