 ...
 * License:     Apache-2.0
 * Description: Implementation of the HASSH Server Fingerprint operation.
 * -----------------------------------------------------------------------------
 */
...
pub struct HASSHServerFingerprint;

impl Operation for HASSHServerFingerprint {
    ...
    fn run(&self, input: Vec<u8>, args: &[ArgValue]) -> Result<Vec<u8>, OperationError> {
        ...
        // Encryption Algorithms Client to Server (skip)
        let _enc_algos_c2s = read_ssh_string(&mut cursor)?;
        // Encryption Algorithms Server to Client
        let enc_algos_s2c = read_ssh_string(&mut cursor)?;
        // MAC Algorithms Client to Server (skip)
        let _mac_algos_c2s = read_ssh_string(&mut cursor)?;
        // MAC Algorithms Server to Client
        let mac_algos_s2c = read_ssh_string(&mut cursor)?;
        // Compression Algorithms Client to Server (skip)
        let _comp_algos_c2s = read_ssh_string(&mut cursor)?;
        // Compression Algorithms Server to Client
        let comp_algos_s2c = read_ssh_string(&mut cursor)?;

        let hassh_str = format!(
            "{};{};{};{}",
            kex_algos, enc_algos_s2c, mac_algos_s2c, comp_algos_s2c
        );
        ...
        match output_format {
            ...
            "Full details" => {
                let details = format!(
                    ...
                    hassh_hash, hassh_str, kex_algos, enc_algos_s2c, mac_algos_s2c, comp_algos_s2c
                );
