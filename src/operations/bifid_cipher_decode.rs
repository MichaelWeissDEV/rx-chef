 ...
 * License:     Apache-2.0
 * Description: Implementation of the Bifid Cipher Decode operation.
 * -----------------------------------------------------------------------------
 */
...
pub struct BifidCipherDecode;

impl Operation for BifidCipherDecode {
        ...
        for pos in structure.iter() {
            if pos == "true" || pos == "false" {
                let coords = [
                    ...
                    trans
                        .chars()
                        .nth(count + trans.len() / 2)
                        .unwrap_or('0')
                        .to_digit(10)
                        .unwrap_or(0) as usize,
                ];

                let c = polybius[coords[0]][coords[1]];
